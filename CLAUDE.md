...
- The relay is content-opaque. It may forward output bytes and attach control generated by the owning agent, but it must not emulate the terminal or derive previews or other message semantics from terminal content.
...
- If you change attach lifecycle semantics, session-state semantics, or `/api/sessions/:id/attach/ws` and `/agent/ws` attach-control messages, update `README.md`, `docs/protocol.md`, `docs/architecture.md`, `CLAUDE.md`, and `AGENTS.md`.
