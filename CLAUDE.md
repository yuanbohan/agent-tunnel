...
- `internal/tunnel/daemon/` owns the explicit background daemon started by `tunnel daemon ...`, including the local control socket, persisted device identity, launcher recipe, doctor/status behavior, and device-side relay connector.
...
- During rollout, the relay still accepts legacy daemon `launch_result` frames that use boolean `accepted: true|false` and normalizes them to the current status-based contract.
