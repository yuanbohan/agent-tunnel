...
- `relay/` owns relay auth, registry, rolling history retention, unread tracking, structured session-state fanout, heartbeat cleanup, and relay HTTP/WebSocket handlers.
...
- The relay is still live-only and monitoring-focused. It lists live sessions, retains a rolling in-memory history buffer per live session, tracks a shared read marker, and forwards multiplexed live updates on one global client socket; it does not create or stop local sessions.
...
- The preferred client foreground receive channel is `GET /api/updates/ws`, which multiplexes output and structured session metadata by `session_id`.
