...
- `cmd/agentunnel` builds the `tunnel` CLI. It launches `claude`, `codex`, or `gemini`, keeps the local terminal interactive, and maintains the in-memory replay history for the current PTY session.
...
- `connector/` is the mandatory outbound connector from a local `tunnel` process to `/agent/ws` on the relay. It authors `seq`, `ts`, and `latest_seq`, uploads live output, and answers proxied `history_request` messages from the relay.
