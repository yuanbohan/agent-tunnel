...
- `relay/` owns relay auth, registry, retained output frame storage, and relay HTTP/WebSocket handlers.
...
- `cmd/relay` requires `AGENTUNNEL_BASIC_USER`, `AGENTUNNEL_BASIC_PASSWORD`, `AGENTUNNEL_AGENT_TOKEN`, and `AGENTUNNEL_REDIS_URL` to start.
...
- The relay is live-only and monitoring-focused for session discovery. It lists live sessions from memory, retains bounded output frame history in Redis per `session_id`, and forwards multiplexed live updates on one global client socket; it does not create or stop local sessions.
...
- `GET /api/sessions/:id/frames` is the standard relay-side recovery path for recently retained output after reconnect, including after agent disconnect or relay restart, until retained history expires.
- Live session state is memory-backed and disappears from `/api/sessions` when the owning agent socket disconnects. Retained frame history is separate, Redis-backed, and currently expires 24 hours after the last retained output for that `session_id`.
