...
- The relay is content-opaque. It may forward output bytes, attach control, snapshot submit-anchor metadata, and live submit-anchor metadata generated by the owning agent, but it must not emulate the terminal or derive previews, anchors, or other message semantics from terminal content. Step 3 broker previews are local-only between `tunnel run` and the daemon.
