...
- the trailing carriage return can create a bounded submit anchor for future fresh attaches
...
- `input_key { "key": "ENTER" }` sends the same carriage return as `input_text { "submit": true }` and can create a bounded submit anchor
