---
title: feat: Add submit anchor index
type: feat
status: completed
date: 2026-04-28
...
# feat: Add Submit Anchor Index

## Overview

Add an agent-local, bounded submit-anchor index so mobile clients can show right-side jump dots for recent local or remote submit Enter events after refresh or reattach. The anchor contract should ride alongside the existing attach snapshot boundary: the agent remains the terminal-state authority, the relay remains content-opaque, and clients still recover through fresh snapshot plus live bytes.
...
Long coding-agent sessions make it hard for a mobile user to find the points where they submitted previous prompts. Tunnel already owns the PTY, structured remote input translation, and the headless terminal mirror used for fresh attach snapshots. This plan adds a small metadata layer at that same ownership boundary so valid recent submit locations can be restored without relay transcript storage or terminal-output parsing.
...
**Origin flows:** F1 Submit and mark an anchor, F2 Reattach and restore jump dots, F3 Anchor expires with scrollback
**Origin acceptance examples:** AE1 submits produce multiple dots, AE2 local and mobile Enter submits are both recoverable when retained, AE3 expired anchors are omitted, AE4 relay remains content-opaque
...
- Mobile app visual rendering and dense-dot interaction polish: implement in the mobile client once this repo exposes the attach metadata contract.
...
- `internal/relay/session/registry.go` routes `attach_ready`, snapshot bytes, `snapshot_done`, and resize controls without inspecting terminal bytes. It should pass through anchor metadata as opaque attach control data.
...
- Keep live attach behavior unchanged for v1: active mobile clients may maintain local provisional dots for their own submits, while the server-side authoritative recovery path is fresh attach and reattach metadata.
...
- How should anchors be delivered? Include optional anchors on `snapshot_done`, preserving the existing attach sequence and avoiding an extra control phase.
...
- Mobile dense-dot visual treatment: belongs to the external mobile client after the attach metadata contract exists.
...
The anchor list is tied to the snapshot it accompanies. A client must apply the snapshot bytes first, then interpret each anchor's `line` against that restored terminal buffer.

---

## Implementation Units

- U1. **Define submit-anchor protocol metadata**

**Goal:** Add the attach metadata shape needed for snapshot-relative submit anchors without changing binary terminal byte packets.

**Requirements:** R1, R2, R5, R7, R8, R10, AE1, AE4
...
**Verification:**
- Protocol tests prove optional anchor metadata can move through JSON control frames without changing attach binary packet behavior.
...
**Requirements:** R1, R3, R4, R12, F1, AE1, AE2
...
- U4. **Pass anchor metadata through relay attach routing**

**Goal:** Let the relay forward submit-anchor metadata from the owning agent to the attached client without interpreting terminal bytes or storing history.

**Requirements:** R5, R7, R8, R10, F2, AE4
...
- Preserve existing slow-client and stale-client behavior for control-message send failures.
...
- Error path: if sending `snapshot_done` with anchors fails, the relay detaches the slow client using the existing slow-client path.
- Error path: `snapshot_done` from a non-owner or for a detached client is ignored as existing lifecycle rules require.

**Verification:**
- Relay tests prove anchors are routed as opaque attach control metadata and do not create relay-owned history.

---

- U5. **Update public attach contract documentation**

**Goal:** Document the new optional submit-anchor metadata and the intended mobile-client interpretation without widening the product promise.

**Requirements:** R2, R5, R8, R9, R10, R11, R12, F2, F3, AE3, AE4
...
**Verification:**
- Docs consistently describe submit anchors as optional snapshot metadata and do not imply transcript replay, Relay terminal emulation, or Codex-specific parsing.

---

## System-Wide Impact

- **Interaction graph:** Mobile attach input flows through the relay attach websocket to the agent connector, while local terminal input flows directly through the session Hub. Anchor creation happens when the bound connector observes Enter-bearing Hub input and records into the mirror; anchors return on fresh attach through agent websocket, relay registry, and client attach websocket.
- **Error propagation:** Anchor metadata should follow existing `snapshot_done` control-message failure behavior. Slow or closed clients are detached through existing paths.
...
- **API surface parity:** Client attach docs, protocol types, relay routing, and connector behavior must agree on optional `snapshot_done.submit_anchors`.
- **Integration coverage:** Protocol unit tests are not enough; connector and relay websocket tests must prove metadata reaches attach clients in the correct lifecycle phase.
...
- This changes the public attach websocket message contract, so `docs/api.md`, `docs/protocol.md`, `docs/tui-attach-flow.md`, `docs/architecture.md`, `README.md`, and `CLAUDE.md` need coordinated updates.
- No PostgreSQL schema, Docker deployment, operator workflow, or relay persistence change is expected.
- Mobile client work is a follow-up outside this repo: render dots from `snapshot_done.submit_anchors`, interpret `line` after applying snapshot bytes, and keep local provisional dots for active-session submits if desired.
