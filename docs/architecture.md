...
Local-terminal input and remote attach input share the same PTY-owner translation path: `input_text { submit: true }` appends one trailing carriage return as part of one serialized operation for that session, and `input_key { key: "ENTER" }` sends the same carriage return semantics. This keeps terminal behavior close to the PTY owner and avoids embedding terminal emulation inside the relay.
...
10. If a client later calls `POST /api/sessions/:id/stop`, the relay sends `stop_session` to the owning agent, removes the live session from discovery, and closes active attaches with `session_stopped`.
