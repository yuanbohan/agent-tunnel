...
Cross-repository SSOT scope is limited to the mobile-visible session metadata and convergence semantics that appear in `agent-tunnel-protocols:docs/protocol.md`; local socket lifecycle, broker ownership, and process mechanics remain implementation details of this repository. This file is a Go implementation note, not a protocol SSOT.
