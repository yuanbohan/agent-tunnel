...
JSON was chosen over CBOR / protobuf for phase 1 to keep tcpdump output and customer-support log dumps human-readable. The
protocol contract is anchored to SSOT `agent-tunnel-protocols`; unknown JSON fields are ignored and unknown frame types are tolerated.
Receivers MUST tolerate future protocol deltas by:
- ignoring unknown JSON object members
- tolerating unknown frame type values

Phase-2 may introduce a CBOR profile under `protocol_version = 2` if Android JSON-parse cost becomes load-bearing (see `../contract.md` open TODOs).
