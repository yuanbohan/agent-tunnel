...
- clients should ignore unknown control fields for forward compatibility
...
- `input_text` with `submit: false` does not append Enter
- `input_key { "key": "ENTER" }` sends the same carriage return
