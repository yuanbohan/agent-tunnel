import{c as m,C as f,e as v,d as y}from"./protocol-ePDsCRtM.js";async function _(){const e=await fetch("/api/sessions",{credentials:"same-origin"});if(!e.ok)throw new Error(`failed to load sessions: ${e.status}`);return e.json()}function w(e,s){return`${e.protocol==="https:"?"wss:":"ws:"}//${e.host}/api/sessions/${encodeURIComponent(s)}/ws`}function $(e){var i,t;const s=((i=e.label)==null?void 0:i.trim())||c(e.launcher),n=c(e.launcher),r=((t=e.last_preview)==null?void 0:t.trim())||"No preview yet";return`
  ...
  `}function C(e){switch(e.trim().toLowerCase()){case"codex":return"CX";case"gemini":return"GM";case"claude":return"CL";default:return"--"}}function c(e){const s=e.trim();return s===""?"Unknown":s.charAt(0).toUpperCase()+s.slice(1)}function g(e){if(!e)return"--";const s=Date.parse(e);if(Number.isNaN(s))return"--";const n=Math.max(0,Math.floor((Date.now()-s)/1e3));return n<60?"now":n<3600?`${Math.floor(n/60)}m`:n<86400?`${Math.floor(n/3600)}h`:`${Math.floor(n/86400)}d`}function a(e){return e.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;").split('"').join("&quot;")}function M(e){if(e==="/"||e==="")return{kind:"dashboard"};const s=e.match(/^\/sessions\/([^/]+)$/);return s?{kind:"session",sessionId:decodeURIComponent(s[1])}:{kind:"dashboard"}}function b(e){return!e}function l(e){return e?"Input on":"Read-only"}function d(e){return e?"input-chip input-chip--enabled":"input-chip"}const p=document.getElementById("relay-root"),u=M(window.location.pathname);u.kind==="dashboard"?L():I(u.sessionId);async function L(){p.innerHTML=`
